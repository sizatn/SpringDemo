package com.sizatn.springdemo.common.log;
...
public final class DailyRollingLogInDirAppender extends DailyRollingFileAppender {
	...
	/**
	 * default backup log file number is 10
	 */
	...
	public void doRollOver() {
		...
		if (maxBackupIndex > 0) {
			// delete old file, but the number of file less than backup file number, keep latest file
			file = new File(cycleFileName + '.' + maxBackupIndex);
			if (file.exists()) {
				file.delete();
			}

			// {(maxBackupIndex - 1), ..., 2, 1} to {maxBackupIndex, ..., 3, 2}
